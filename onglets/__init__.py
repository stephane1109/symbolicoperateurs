...
def rendu_donnees_brutes(
    ...
    stats_df = pd.DataFrame(
        sorted(count_connectors(combined_text, filtered_connectors).items(), key=lambda x: x[1]),
        columns=["connecteur", "occurrences"],
    )
    stats_df["label"] = stats_df["connecteur"].map(filtered_connectors)
